import json
with open('orders_july.json', 'r') as my_file:
...
#2 Какой номер заказа с самым большим количеством товара
max_quantity = 0
max_order = ''
for order_num, orders_data in orders.items():
...
for order_num, orders_data in orders.items():
    ...
    if quantity > max_quantity:
        ...
        max_quantity = quantity
print(f'Больше всего заказов : {max_quantity} было сделанно: {date}')
...
for order, data in orders.items():
    sum_all += data['price']
    count += 1
print(f'Cредняя стоимость заказов в июле :{sum_all/count}')
